...
[ -f /bin/pacman ] sudo pacman -Sy ripgrep --noconfirm || ERR "could not install ripgrep..."
[ -f /bin/apt ] sudo apt install ripgrep || ERR "could not install ripgrep..."

if [ $ans -eq 2 ]; then
	running "Setting up nvim..."
	[ -d "$configs" ] && {
		running "making backup..."
		mv "$configs" "$HOME/.config/nvim_backup"
	}
